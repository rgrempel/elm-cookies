...
Elm.Cookies = Elm.Cookies || {};
Elm.Cookies.make = function (_elm) {
   "use strict";
   _elm.Cookies = _elm.Cookies || {};
   if (_elm.Cookies.values)
   return _elm.Cookies.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$Cookies = Elm.Native.Cookies.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm),
   $Task = Elm.Task.make(_elm),
   $Time = Elm.Time.make(_elm);
   var dateToUTCString = $Native$Cookies.dateToUTCString;
   var setString = $Native$Cookies.setString;
   var setWithOptions = F3(function (options,
   key,
   value) {
      return function () {
         var andThen = $Basics.flip($Maybe.andThen);
         var handlers = _L.fromArray([$Basics.always($Maybe.Just(A2($Basics._op["++"],
                                     $Http.uriEncode(key),
                                     A2($Basics._op["++"],
                                     "=",
                                     $Http.uriEncode(value)))))
                                     ,function ($) {
                                        return andThen(function (path) {
                                     ...
                                     ,function ($) {
                                        return andThen(function (domain) {
                                           return $Maybe.Just(A2($Basics._op["++"],
                                           "domain=",
                                           $Http.uriEncode(domain)));
                                        })(function (_) {
                                           return _.domain;
                                        }($));
                                     }
                                     ,function ($) {
                                        return andThen(function (age) {
                                           return $Maybe.Just(A2($Basics._op["++"],
                                           "age=",
                                           $Basics.toString($Time.inSeconds(age))));
                                        })(function (_) {
                                           return _.maxAge;
                                        }($));
                                     }
                                     ,function ($) {
                                        return andThen(function (expires) {
                                           return $Maybe.Just(A2($Basics._op["++"],
                                           "expires=",
                                           dateToUTCString(expires)));
                                        })(function (_) {
                                           return _.expires;
                                        }($));
                                     }
                                     ,function ($) {
                                        return andThen(function (secure) {
                                           return secure ? $Maybe.Just("secure") : $Maybe.Nothing;
                                        })(function (_) {
                                           return _.secure;
                                        }($));
                                     }]);
         var cookieStrings = A2($List.filterMap,
         function (f) {
            return f(options);
         },
         handlers);
         return setString(A2($List.foldl,
         F2(function (x,y) {
   ...
   var defaultOptions = {_: {}
                        ...
                        ,expires: $Maybe.Nothing
                        ,maxAge: $Maybe.Nothing
                        ,path: $Maybe.Nothing
                        ,secure: $Maybe.Nothing};
   ...
   var set = setWithOptions(defaultOptions);
   var cookieString2Dict = function () {
      var addKeyValueToDict = function (keyValueList) {
         return function () {
            switch (keyValueList.ctor)
            {case "::":
               switch (keyValueList._1.ctor)
                 {case "::":
      ...
      var addCookieToDict = function ($) {
         return addKeyValueToDict($List.map($Http.uriDecode)($String.split("=")($String.trim($))));
      };
      return function ($) {
         return A2($List.foldl,
         addCookieToDict,
         $Dict.empty)($String.split(";")($));
      };
   }();
   var getString = $Native$Cookies.getString;
   var get = A2($Task.map,
   cookieString2Dict,
   getString);
   _elm.Cookies.values = {_op: _op
                         ,get: get
                         ,set: set
...
Elm.Test.make = function (_elm) {
   ...
   "Cookies",
   _L.fromArray([$ElmTest$Test.test("simple set then get")(A2($ElmTest$Assertion.assertEqual,
   "ob",
   "ob"))]));
   var performTasks = Elm.Native.Task.make(_elm).performSignal("performTasks",
   $Signal.constant($Task.succeed({ctor: "_Tuple0"})));
